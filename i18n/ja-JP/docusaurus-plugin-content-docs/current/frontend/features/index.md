...
Menu Item Reservation はその代表例です。KFC Japan の Christmas Ordering フローは、独自 POS である **C-BOH** の在庫 API と timeslot API に依存しています。同等の API を持たない市場では、その連携を整備したうえで機能を有効化する必要があります。
